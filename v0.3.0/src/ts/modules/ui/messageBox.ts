import { MVVMDialog, IDialogOptions, TDialogButtons, TDialogResults } from '@webkit/core/MVVMDialog.class';
...
export class MessageBox extends MVVMDialog {
	...
	public static create(title: string, text: string, buttons: any, callback?: any, opts?: IMessageBoxOptions): MessageBox {    
        let oMessageBox_: MessageBox = new MessageBox(title, text, buttons, callback, opts);
        oMessageBox_.show();
        return oMessageBox_;
	}
	...
	public static popin(text: string, messageType?: string, timeout?: number): void {
		...
		let localeText: string = app.i18n.getString(text)

		timeout = !isNaN(timeout) ? timeout : (messageType == 'error' ? 0 : 2000)
		messageType = messageType || 'info'

		let $template = $(MessageBox._popinTemplate)
		let close = function() {
