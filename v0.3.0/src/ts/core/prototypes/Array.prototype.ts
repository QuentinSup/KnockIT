...
Array.prototype.contains = function(o: any): boolean {
	...
	for(let i: number = 0, len = o.length; i < len; i++) {
...
Array.prototype.union = function(o: any[]): any[] {
    let ary: any[] = [];
    let tab1: any[] = this.length > o.length?o:this;
    let tab2: any[] = this.length > o.length?this:o;
    for(let i: number = 0, len = tab1.length; i < len; i++) {
...
Array.prototype.indexOfStr = function (elt: any, from: number = 0): number {
    ...
    let i: number = from < 0 ? Math.max(0, len + from) : from
    let str = elt.toLowerCase()
    for (; i < len; i++) {
...
Array.prototype.pushOnce = function(elt: any, caseSensitive: boolean = false): boolean {
    let eltUCase = (caseSensitive && typeof(elt) == "string")?elt.toUpperCase():elt
    let m: any = caseSensitive?this.map(function(v) { return typeof(v) == "string"?v.toUpperCase():v }):this
...
Array.prototype.shake = function(): any[] {
    ...
    let t_: any[] = [].concat(this);
    let r_: any[] = [];
    
    while(t_.length > 0) {
        let index_:number = Math.floor(Math.random() * this.length);
